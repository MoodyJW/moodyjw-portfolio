...
export class TextareaComponent {
  ...
  readonly charCountText = computed(() => {
    ...
    if (!max || !this.showCharCount()) return '';
  ...
  readonly showFooter = computed(() => {
    return !!this.helperText() || (this.showCharCount() && !!this.maxLength());
  });
  ...
  readonly shouldShowCharCount = computed(() => {
    return this.showCharCount() && !!this.maxLength();
  });
  ...
  readonly helperTextId = computed(() => {
    ...
    return helperText ? `textarea-helper-${this._generateId()}` : undefined;
  });
  ...
  readonly labelId = computed(() => {
    ...
    return label ? `textarea-label-${this._generateId()}` : undefined;
  });
  ...
  readonly computedAriaDescribedBy = computed(() => {
    ...
    if (helperTextId) parts.push(helperTextId);
    if (userDescribedBy) parts.push(userDescribedBy);
  ...
  private _calculateMinHeight(): number | undefined {
    ...
    if (minRowsValue === undefined) return undefined;
  ...
  private _safeGetComputedStyle(element: HTMLElement): CSSStyleDeclaration | null {
    const g = globalThis as typeof globalThis & { getComputedStyle?: (e: Element) => CSSStyleDeclaration };
    if (!g || typeof g.getComputedStyle !== 'function') return null;
    try {
