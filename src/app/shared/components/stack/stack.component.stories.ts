...
const createStackItem = (text: string, color = 'var(--color-surface)') => `
  <div style="
    padding: 1rem 1.5rem;
    background: ${color};
    ...
    template: `
      <div style="display: flex; flex-direction: column; gap: 3rem;">
        <div>
          ...
          <app-stack spacing="none">
            ${Array.from({ length: 3 }, (_, i) => createStackItem(`Item ${i + 1}`)).join('')}
          </app-stack>
        </div>
    ...
    template: `
      <div style="display: flex; flex-direction: column; gap: 2rem;">
        <div>
          ...
          <app-stack spacing="md" align="start">
        ...
        <div>
          ...
          <app-stack spacing="md" align="end">
            ${createStackItem('Short')}
            ${createStackItem('Medium length text')}
            ${createStackItem('This is a much longer item with more text')}
          </app-stack>
      ...
      <div style="display: flex; flex-direction: column; gap: 2rem;">
        <div>
          ...
          <div style="height: 150px; border: 2px dashed var(--color-border); border-radius: var(--border-radius); padding: 1rem;">
            <app-stack direction="horizontal" spacing="md" align="start">
        ...
        <div>
          ...
          <div style="height: 150px; border: 2px dashed var(--color-border); border-radius: var(--border-radius); padding: 1rem;">
            <app-stack direction="horizontal" spacing="md" align="end">
              ${createStackItem('Short', 'var(--color-primary)')}
              ${createStackItem('Medium<br>height', 'var(--color-primary)')}
              ${createStackItem('Tall<br>item<br>here', 'var(--color-primary)')}
            </app-stack>
      ...
      <div style="display: flex; flex-direction: column; gap: 2rem;">
        <div>
          ...
          <div style="height: 100px; border: 2px dashed var(--color-border); border-radius: var(--border-radius); padding: 1rem;">
            <app-stack direction="horizontal" spacing="md" justify="start">
              ${Array.from({ length: 3 }, (_, i) => createStackItem(`Item ${i + 1}`)).join('')}
            </app-stack>
          </div>
        </div>
        ...
        <div>
          ...
          <div style="height: 100px; border: 2px dashed var(--color-border); border-radius: var(--border-radius); padding: 1rem;">
            <app-stack direction="horizontal" spacing="md" justify="center">
              ${Array.from({ length: 3 }, (_, i) => createStackItem(`Item ${i + 1}`)).join('')}
            </app-stack>
          </div>
        </div>
        ...
        <div>
          ...
          <div style="height: 100px; border: 2px dashed var(--color-border); border-radius: var(--border-radius); padding: 1rem;">
            <app-stack direction="horizontal" spacing="none" justify="space-between">
              ${Array.from({ length: 3 }, (_, i) => createStackItem(`Item ${i + 1}`)).join('')}
            </app-stack>
          </div>
        </div>
        ...
        <div>
          ...
          <div style="height: 100px; border: 2px dashed var(--color-border); border-radius: var(--border-radius); padding: 1rem;">
            <app-stack direction="horizontal" spacing="none" justify="space-around">
              ${Array.from({ length: 3 }, (_, i) => createStackItem(`Item ${i + 1}`)).join('')}
            </app-stack>
          </div>
        </div>
        ...
        <div>
          ...
          <div style="height: 100px; border: 2px dashed var(--color-border); border-radius: var(--border-radius); padding: 1rem;">
            <app-stack direction="horizontal" spacing="none" justify="space-evenly">
              ${Array.from({ length: 3 }, (_, i) => createStackItem(`Item ${i + 1}`)).join('')}
            </app-stack>
          </div>
        </div>
    ...
    template: `
      <div style="display: flex; flex-direction: column; gap: 3rem;">
        <div>
          ...
          <app-stack spacing="lg" [divider]="true">
            ${Array.from({ length: 4 }, (_, i) => createStackItem(`Section ${i + 1}`)).join('')}
          </app-stack>
        </div>

        <div>
          <h4 style="margin: 0 0 0.5rem 0;">Horizontal Stack with Dividers</h4>
          <app-stack direction="horizontal" spacing="lg" [divider]="true">
            ${Array.from({ length: 4 }, (_, i) => createStackItem(`Item ${i + 1}`)).join('')}
          </app-stack>
        </div>
      </div>
    `,
  ...
  render: () => ({
    template: `
      <div style="max-width: 600px; border: 2px dashed var(--color-border); border-radius: var(--border-radius); padding: 1rem;">
        ...
        <app-stack direction="horizontal" spacing="md" [wrap]="true">
          ${Array.from({ length: 12 }, (_, i) => createStackItem(`Item ${i + 1}`)).join('')}
        </app-stack>
      </div>
    `,
  }),
...
export const FormActionsExample: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Real-world example: Form with vertical stack for fields and horizontal stack for actions.',
      },
    },
  },
};
  ...
  render: () => ({
    template: `
      <div style="padding: 1.5rem; background: var(--color-surface); border-radius: var(--border-radius);">
        <app-stack direction="horizontal" spacing="xl" align="center">
...
export const AccessibilityNotes: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Comprehensive accessibility features and best practices for using the stack component.',
      },
    },
  },
};
